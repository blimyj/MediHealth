...
class LoginScreen extends Component {
    ...
    dateTimeSplitter(dateTimeObj){
        ...
        var date = new Date("${dateTimeObj.getFullYear()}-${dateTimeObj.getMonth()}-${dateTimeObj.getDate()}");
        var time = new Date("0000 ${dateTimeObj.getHours()}:${dateTImeObj.getMinutes()}");
        return [date, time];
    }
    ...
    initialiseUserData(uid_no){
        ...
        firebase
            ...
            .set({
                    //Profile
                    Profile: {
                        profilePic: "https://media.licdn.com/dms/image/C5103AQEiJL0AgWj5KQ/profile-displayphoto-shrink_800_800/0?e=1566432000&v=beta&t=MaAA-eyV5MXKmgj4rRqSfKE8fwGDtjkkVn-EMruGzKA",
                        displayName: "Hao Jun",
                        age: 22,
                        weight: 20,
                        height: 190,
    ...
    signUp(email, password) {
        alert("Creating your account! Please wait for the next alert. //This to be changed to a modal screen popup with loading animation.")
    ...
    login(email, password) {
            alert("Logging you in! Please wait for the next alert. //This to be changed to a modal screen popup with loading animation.")
    ...
    logout() {
        alert("Logging you out! Please wait for the next alert. //This to be changed to a modal screen popup with loading animation.")
        firebase.auth().signOut()
            .then( () => {
                alert("You've been logged out!")
            },
            () => {
                alert("Unknown Logout Error. \n Beware: You may not have been logged out.")
            });
        this.setState({ password: '' });
        this.setState({ email: '' });
				...
				>
                    ...
                    <View style={styles.ButtonsContainer}>
                        ...
                        <View style={styles.ButtonContainer}>
                            ...
                            >
                                <Text style={styles.bigButtonText}>Display User</Text>
                            </TouchableOpacity>
                        </View>
